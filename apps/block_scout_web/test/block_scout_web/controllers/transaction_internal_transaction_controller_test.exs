defmodule ExplorerWeb.TransactionInternalTransactionControllerTest do
  ...
  import ExplorerWeb.WebRouter.Helpers, only: [transaction_internal_transaction_path: 3]
  ...
  describe "GET index/3" do
    test "with missing transaction", %{conn: conn} do
      ...
      conn = get(conn, transaction_internal_transaction_path(ExplorerWeb.Endpoint, :index, hash))

      assert html_response(conn, 404)
    end

    test "with invalid transaction hash", %{conn: conn} do
      conn = get(conn, transaction_internal_transaction_path(ExplorerWeb.Endpoint, :index, "nope"))
    ...
    test "includes transaction data", %{conn: conn} do
      ...
      conn = get(conn, transaction_internal_transaction_path(ExplorerWeb.Endpoint, :index, transaction.hash))
    ...
    test "includes internal transactions for the transaction", %{conn: conn} do
      ...
      path = transaction_internal_transaction_path(ExplorerWeb.Endpoint, :index, transaction.hash)
    ...
    test "returns next page of results based on last seen internal transaction", %{conn: conn} do
      ...
      conn =
        get(conn, transaction_internal_transaction_path(ExplorerWeb.Endpoint, :index, transaction.hash), %{
