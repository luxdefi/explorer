defmodule ExplorerWeb.Tokens.HolderControllerTest do
  ...
  describe "GET index/3" do
    test "with invalid address hash", %{conn: conn} do
      conn = get(conn, token_holder_path(ExplorerWeb.Endpoint, :index, "invalid_address"))
    ...
    test "with a token that doesn't exist", %{conn: conn} do
      ...
      conn = get(conn, token_holder_path(ExplorerWeb.Endpoint, :index, address.hash))
    ...
    test "successfully renders the page", %{conn: conn} do
      ...
      conn =
        get(
          conn,
          token_holder_path(ExplorerWeb.Endpoint, :index, token.contract_address_hash)
        )
